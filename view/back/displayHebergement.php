...
$sort_order = isset($_GET['sort']) ? $_GET['sort'] : 'asc'; // Default sorting order

if (isset($_GET['search'])) {
    $search = $_GET['search'];
    $show = "SELECT * FROM hebergement WHERE nom LIKE :search ORDER BY nom $sort_order LIMIT :limit OFFSET :offset";
    $r = $db->prepare($show);
    ...
    $show = "SELECT * FROM hebergement ORDER BY nom $sort_order LIMIT :limit OFFSET :offset";
    $r = $db->prepare($show);
    $r->bindParam(':limit', $limit, PDO::PARAM_INT);
    $r->bindParam(':offset', $starting_limit, PDO::PARAM_INT);
    $r->execute();
    ...
    <tbody>
        ...
        while ($res = $r->fetch(PDO::FETCH_ASSOC)) {
            echo '<tr>
                ...
                <td>' . $res['service'] . '</td>
                <td>
                    <button class="btn btn-primary"><a href="update.php?updateid=' . $res['id_heb'] . '" class="text-light">Update</a></button>
...
for ($i = 1; $i <= $total_pages; $i++) {
    echo '<a href="?page=' . $i . '&sort=' . $sort_order . '" class="links">' . $i.'</a>';
}
?>
<div>
...
<html>
<head>
  <title>Table with QR Code</title>
  <style>
    .container {
      display: flex;
    ...
    <div class="row">
      <div class="col-md-8">
        <div class="table-responsive">
          ...
          $limit = 5;
          $query = "SELECT count(*) FROM hebergement";
          // Rest of the PHP code for table goes here
          ?>
          <form method="GET" action="">
            <!-- Search form goes here -->
          </form>
          <table class="table align-items-center">
            <!-- Table headers and rows code goes here -->
          </table>
          <?php
          for ($i = 1; $i <= $total_pages; $i++) {
            echo '<a href="?page=' . $i . '&sort=' . $sort_order . '" class="links">' . $i . '</a>';
